...
#SBATCH --job-name=dere(de_novo) 		# Assign an short name to your job
...
# srun --partition=genetics_1 --nodes=1 --ntasks=1 --cpus-per-task=1 --mem=30 --time=01:30:00 --export=ALL --pty bash -i
...
species_directory=dere_RepeatModeler
...
# LOG FILE
log_file=dere_de_novo_log.txt
date_time=date

# BuildDatabase
# THIS FILE IS A VARIABLE INPUT FOR BuildDatabase
fasta_file=dere_nanopore
raw_file=nanopore/dere.pass.minimap2.racon.x3.pilon.x3.fasta
...
# RepeatModeler
# LOG FILE
log_file=dere_run.log

# UCLUST
# OUTPUT FILE NAME
uclust_output=uclusted_dere.fasta

# BLAST
# PEPTIDE SEQUENCE
peptide_seqs=melanogaster-all-translation-r6.24.fasta
#BLASTX OUTPUT
blastx_output=dere_blastx.out
...
dir_RepeatMasker=dere_RepeatMasker
repeatmasker_gff=dere_RepeatMasker.gff
repeatmasker_shortened_gff=dere_RepeatMasker_shortened.gff
repeatmasker_shortened_merged_bed=dere_RepeatMasker_shortened_merged.bed
...
summary_file=dere_summary_file.txt
transposon_freq=transposon_freq.gff
dir_transposon_freq=dere_transposon_freq
...
sleep 3
cd /scratch/cpr74/2019Spring/drosophila_classifications/dere_nanopore_2019_03_05
...
cp ./$dir_RepeatMasker/$uclust_no_genes.out.gff ./$repeatmasker_gff
...
python3 ../repeat_masker_shortener.py -i $repeatmasker_gff -o $repeatmasker_shortened_gff; sleep 5
...
python3 ../repeat_masker_parser.py -r $repeatmasker_shortened_merged_bed -t $threshold -u $uclust_no_genes -p $uclust_postmatch -s $uclust_summary; sleep 5
...
~/pkg/RepeatMasker/RepeatMasker -e ncbi -pa 28 -nolow -norna -no_is -dir $dir_transposon_freq -lib $uclust_postmatch -gff ./../$fasta_file; sleep 5
cp ./$dir_transposon_freq/$fasta_file.out.gff ./$transposon_freq; sleep 5
...
# condense classes
python3 ../class_condensor.py -i ./final_output -o ./../../identified_TEs/dere_condensed_classes.txt -s dere
...
trf_bp="$(cat dere_nanopore_corrected.2.7.7.80.10.50.500.dat | grep -v "Sequence" | grep -v "Parameters" | grep -v "\n" | grep -v -e '^$' | cut -f1,2 | awk '{ print $2-$1 }' | awk '{total += $0} END {print total}')"
echo "Simple Repeat bps in $species genome = $trf_bp ." >> $summary_file
assembly_size="$(grep -v ">" ../dere_nanopore_corrected | wc | awk '{print $3-$1}')"
